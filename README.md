...
可以将当前的整理成果（折叠/展开、隐藏、删除、注释）保存为 JSON 文件，方便下次快速恢复：

**保存工作状态：**
- 整理好树的结构后，点击「💾 保存状态」按钮下载 `treeview-state.json` 文件
- 可以将此文件放入项目版本控制，与团队共享

**加载保存的状态：**

方式 1：在浏览器中加载（最灵活）
- 加载一个新的 tree 文件到 treeview
- 点击「📥 导入状态」选择之前保存的 JSON 状态文件
- 页面会自动应用折叠、隐藏、删除、注释等全部状态

方式 2：通过命令行加载（最快）
```bash
tree | ./treeview -s state.json
# 或
tree | ./treeview --state state.json
```
这样会自动加载保存的状态，打开浏览器时就能看到上次的整理结果。
